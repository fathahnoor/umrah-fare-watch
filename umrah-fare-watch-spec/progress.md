...
- Next: M5-M7 (provider real) menunggu user mendaftar **SerpAPI** (serpapi.com) - satu-satunya jalur live yang mencakup rute Indonesia setelah Duffel (negara), Amadeus self-service (ditutup), Kiwi (invite-only), dan Hotellook (ditutup) gugur. Opsional: FX_API_KEY gratis dari exchangerate.host. Begitu SERPAPI_API_KEY masuk: smoke test server-side route-aware untuk CGK->JED, baru flip gate dan verifikasi live di preview.
